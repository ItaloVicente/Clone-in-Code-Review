...
public class LargeDocumentEditorsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage, Listener {
	...
	protected void fillEditorTable() {
		...
		if (association != null) {
			...
			if (association.promptSize != null) {
				...
				String[] text = { "dialog prompt", sizeDisplayString }; //$NON-NLS-1$
				item.setText(text);
			}
		}
	}
	...
	public void promptForEditor() {
		...
		dialog.setMessage(NLS.bind(WorkbenchMessages.Choose_the_editor_for_file,
		...
		if (dialog.open() == Window.OK) {
			...
			if (editor != null) {
				...
				String[] text = { editor.getLabel(), "1 GB" }; //$NON-NLS-1$
				item.setText(text);
				item.setImage(getImage(editor));
				editorTable.setSelection(i);
				editorTable.setFocus();
				EditorForSize editorForSize = new EditorForSize(editor, "1 GB"); //$NON-NLS-1$
				getSelectedLargeDocumentAssociation().editors.add(editorForSize);
			}
		}
	}
