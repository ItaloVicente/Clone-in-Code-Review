...
public class LargeFileLimitsPreferenceHandler {
	...
	private Optional<String> getEditorForPath(IPath inputPath) {
		...
		String editorId = fileLimit.editorId;
		boolean isPromptPreferenceValue = isPromptPreferenceValue(fileLimit.editorId);
	...
	private void replaceLimitForLargeFile(IPath path, FileLimit oldLimit, FileLimit newLimit) {
		try {
			...
			WorkbenchPlugin.log("Exception occurred while replacing large file editor preference", e); //$NON-NLS-1$
		}
	}
