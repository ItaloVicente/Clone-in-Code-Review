...
public class AboutDialogE4 extends TrayDialog {
	...
	protected void configureShell(final Shell newShell) {
		...
		newShell.setText(NLS.bind(E4DialogMessages.AboutDialog_shellTitle, productName));
	...
	protected Control createDialogArea(final Composite parent) {
		...
		if (product != null) {
			...
			if (imageDescriptor != null && JFaceResources.getImage(productName) == null) {
				JFaceResources.getImageRegistry().put(productName, imageDescriptor);
			}

			aboutImage = JFaceResources.getImageRegistry().get(productName);
			if (aboutImage == null || aboutImage.getBounds().width <= MAX_IMAGE_WIDTH_FOR_TEXT) {
				String aboutTextProperty = ProductProperties.getAboutText(product);
