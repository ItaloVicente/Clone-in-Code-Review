...
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_ColumnHeader_Editor;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_ColumnHeader_Size;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_DefaultPreference_Label;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_DisabledPageText;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_EditorDialog_Label_DocumentSize;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_EditorDialog_Label_Editor;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_EditorDialog_Radio_Prompt;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_EditorDialog_Radio_Specific;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_EditorDialog_Title;
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeDocumentEditorsPreferencePage_EditorTable_DialogPrompt;
...
public class LargeDocumentEditorsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage, Listener {
	...
	private static final String DIALOG_PROMPT_EDITOR_TABLE_TEXT = LargeDocumentEditorsPreferencePage_EditorTable_DialogPrompt;
	...
	private Text defaultPreferenceDocumentSize;
	...
	protected Control createContents(Composite parent) {
		boolean isLegacyPreferenceSet = LargeDocumentPreferenceHandler.isLargeDocumentLegacyPreferenceSet();
	...
	private static void createDisabledPreferencePageContents(Composite pageComponent) {
		...
		String disabledPageText = NLS.bind(LargeDocumentEditorsPreferencePage_DisabledPageText,
	...
	private void createPreferencePageContents(Composite pageComponent) {
		...
		editorColumn.setText(LargeDocumentEditorsPreferencePage_ColumnHeader_Editor);
		editorColumn.setWidth(300);
		TableColumn sizeColumn = new TableColumn(editorTable, SWT.NONE);
		sizeColumn.setText(LargeDocumentEditorsPreferencePage_ColumnHeader_Size);
		...
		enableDefaultPreference.setText(LargeDocumentEditorsPreferencePage_DefaultPreference_Label);
		defaultPreferenceDocumentSize = createDocumentSizeText(groupComponent);
	...
	private void updateDefaultPreferenceWidgetsEnabledState() {
		...
		defaultPreferenceDocumentSize.setEnabled(enabled);
		defaultPreferenceUnit.setEnabled(enabled);
	}

	private void setDefaultPreferenceWidgetValues() {
		String defaultPreferenceValue = LargeDocumentPreferenceHandler.getDefaultPreferenceValues();
		if (defaultPreferenceValue == null) {
			enableDefaultPreference.setSelection(false);
		} else {
			enableDefaultPreference.setSelection(true);
			SizeAndUnit sizeAndUnit = getSizeAndUnit(defaultPreferenceValue);
			defaultPreferenceDocumentSize.setText(sizeAndUnit.sizeString);
	...
	private void updateDefaultPreference() {
		...
		if (isDefaultPreferenceEnabled) {
			String sizeInBytes = getSizeInBytes(defaultPreferenceDocumentSize, defaultPreferenceUnit);
			LargeDocumentPreferenceHandler.setDefaultPreferenceValue(sizeInBytes);
		} else {
			LargeDocumentPreferenceHandler.removeDefaultPreference();
		}
	}
	...
	public void promptForEditor() {
		...
		if (editorForSizeDialog.open() == Window.OK) {
			...
			String sizeText = editorForSizeDialog.sizeInBytes;
	...
	private static String getDisplayString(String sizeInBytes) {
		SizeAndUnit sizeAndUnit = getSizeAndUnit(sizeInBytes);
		String sizeString = formatSizeString(sizeAndUnit.sizeString, sizeAndUnit.unitIndex);
		return sizeString;
	}

	private static SizeAndUnit getSizeAndUnit(String sizeInBytes) {
	...
	private static String parseSize(String sizeString) {
		...
		for (int i = 0; i < SIZE_UNITS.length; ++i) {
			String sizeSuffix = SIZE_UNITS[i];
	...
	private static Text createDocumentSizeText(Composite parent) {
	...
	private static String getSizeInBytes(Text documentSizeText, Combo sizeUnitCombo) {
		int sizeUnitSelectionIndex = sizeUnitCombo.getSelectionIndex();
		String documentSizeString = documentSizeText.getText();
		String sizeString = formatSizeString(documentSizeString, sizeUnitSelectionIndex);
		return sizeString;
	}

	private static String formatSizeString(String documentSizeString, int unitIndex) {
		String sizeString = documentSizeString;
		if (unitIndex != BYTE_UNIT_INDEX) {
			String unitText = SIZE_UNITS[unitIndex];
			sizeString += " " + unitText; //$NON-NLS-1$
		}
		return sizeString;
	}
	...
	private static class EditorForSizeDialog extends Dialog {
		...
		EditorForSizeDialog(Shell shell) {
			super(shell);
			sizeInBytes = formatSizeString(DEFAULT_SIZE_STRING, MEGABYTE_UNIT_INDEX);
		...
		protected void configureShell(Shell newShell) {
			...
			newShell.setText(LargeDocumentEditorsPreferencePage_EditorDialog_Title);
		}
		...
		protected Control createDialogArea(Composite parent) {
			...
			editorTypeGroup.setText(LargeDocumentEditorsPreferencePage_EditorDialog_Label_Editor);
			editorTypeGroup.setLayout(new FillLayout(SWT.VERTICAL));
			promptEditorOnOpen = new Button(editorTypeGroup, SWT.RADIO);
			promptEditorOnOpen.setText(LargeDocumentEditorsPreferencePage_EditorDialog_Radio_Prompt);
			promptEditorOnOpen.setSelection(true);
			chooseEditorDialog = new Button(editorTypeGroup, SWT.RADIO);
			chooseEditorDialog.setText(LargeDocumentEditorsPreferencePage_EditorDialog_Radio_Specific);
			chooseEditorDialog.setSelection(false);
			Group documentSizeGroup = new Group(contents, SWT.BORDER);
			documentSizeGroup.setText(LargeDocumentEditorsPreferencePage_EditorDialog_Label_DocumentSize);
			documentSizeGroup.setLayout(new FillLayout(SWT.HORIZONTAL));
			documentSize = createDocumentSizeText(documentSizeGroup);
			Combo sizeUnitCombo = createSizeUnitCombo(documentSizeGroup);
			sizeUnit = sizeUnitCombo;
			return contents;
		}
		...
		protected void okPressed() {
			...
			sizeInBytes = getSizeInBytes(documentSize, sizeUnit);
			super.okPressed();
		}
	}
