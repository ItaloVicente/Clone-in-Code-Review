...
public class LargeFileLimitsPreferenceHandler {
	...
	private final Map<String, FileLimit[]> preferencesCache;
	...
	public static FileLimit[] getFileLimitsForExtension(String fileExtension) {
		String preferenceName = getPreferenceNameForExtension(fileExtension);
		FileLimit[] preferenceValues = getPreferenceValues(preferenceName);
	...
	public static void setFileLimitsForExtension(String fileExtension, FileLimit[] fileLimits) {
		String preferenceName = getPreferenceNameForExtension(fileExtension);
	...
	String getEditorIdForLargeFile(IPathEditorInput editorInput) {
		...
		try {
			IPath path = editorInput.getPath();
			FileLimit[] fileLimits = getFileLimits(path, preferencesCache);
	...
	private static FileLimit[] getFileLimits(IPath path, Map<String, FileLimit[]> preferencesCache) {
		String fileExtension = path.getFileExtension();
		FileLimit[] preferenceValues = preferencesCache.get(fileExtension);
	...
	private static FileLimit[] getLargeFilePreferenceValues(String fileExtension) {
		FileLimit[] preferenceValues = {};
	...
	private static FileLimit[] getPreferenceValues(String preferenceName) {
		String largeFilePreference = PREFERENCE_STORE.getString(preferenceName);
		List<FileLimit> preferenceValues = new ArrayList<>();
		if (largeFilePreference != null && !largeFilePreference.isEmpty()) {
			String[] values = largeFilePreference.split(PREFERENCE_VALUE_SEPARATOR);
			if (values.length % 2 != 0) {
				...
				for (int i = 0; i < values.length; i += 2) {
					String sizeString = values[i + 0];
					String editorId = values[i + 1];
					try {
						long bytes = Long.parseLong(sizeString);
						FileLimit fileLimit = new FileLimit(editorId, bytes);
						preferenceValues.add(fileLimit);
					} catch (NumberFormatException e) {
						String errorMessage = NLS.bind(
								"Skipped invalid file size value \"{0}\" stored in preference \"{1}\" with value \"{2}\"", //$NON-NLS-1$
								new String[] { sizeString, preferenceName, Arrays.toString(values) });
						WorkbenchPlugin.log(new IllegalArgumentException(errorMessage, e));
					}
				}
			}
		}
		return preferenceValues.toArray(FileLimit[]::new);
	}

	private static void setPreferenceValues(String preferenceName, FileLimit[] fileLimits) {
		StringBuilder preferenceValue = new StringBuilder();
		for (int i = 0; i < fileLimits.length; ++i) {
			FileLimit fileLimit = fileLimits[i];
