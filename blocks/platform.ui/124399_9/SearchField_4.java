			...
			restoreDialogEntriesJob = Job.createSystem("Restore quick access elements", (IProgressMonitor monitor) -> { //$NON-NLS-1$
				try {
					restoreDialogEntries(dialogSettings, display, monitor);
					return Status.OK_STATUS;
	...
	private void restoreDialogEntries(IDialogSettings dialogSettings, Display display, IProgressMonitor monitor)
		...
		if (orderedElements != null && orderedProviders != null && textEntries != null && textArray != null) {
			...
			for (int i = 0; i < elementCount; i++) {
				...
				if (quickAccessProvider != null) {
					QuickAccessElement quickAccessElement = quickAccessElement(display, quickAccessProvider,
							orderedElements[i]);
