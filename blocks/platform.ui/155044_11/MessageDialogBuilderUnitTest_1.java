...
public class MessageDialogBuilderUnitTest {
	...
	public void createCustomDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Do ya really want to?").buttons("Yeah", "Nope");
		MessageDialogObserver dialogObserver = openDialogAndObserve(factory);

		assertEquals("Do ya really want to?", dialogObserver.text);
		assertNull(dialogObserver.image);
		assertEquals(2, dialogObserver.buttons.size());
		assertEquals("Nope", dialogObserver.buttons.get(0));
		assertEquals("Yeah", dialogObserver.buttons.get(1));
	}

	@Test
	public void createsInformationDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Info").type(MessageDialog.INFORMATION);
	...
	public void createsErrorDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Error").type(MessageDialog.ERROR);
	...
	public void createsWarningDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Warning").type(MessageDialog.WARNING);
	...
	public void createsQuestionDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Question").type(MessageDialog.QUESTION);
	...
	public void createsQuestionWithCancelDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Question").type(MessageDialog.QUESTION_WITH_CANCEL);
	...
	public void createsConfirmDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Confirm").type(MessageDialog.CONFIRM);
	...
	public void createsDialogWithMessage() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Info").type(MessageDialog.INFORMATION)
				.message("This is just an information");
	...
	public void createsDialogWithImage() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("This is an info but it shows an error image")
				.type(MessageDialog.INFORMATION).image(errorImage);
	...
	public void createsDialogWithButtonLabels() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Info").type(MessageDialog.INFORMATION).buttons("OK",
				"Cancel", "Maybe", "Unsure");
	...
	public void createsSheetStyleDialog() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Info").type(MessageDialog.INFORMATION).sheet();
	...
	public void createsDialogWithDefaultButton() {
		MessageDialogBuilder factory = MessageDialogBuilder.title("Info").type(MessageDialog.INFORMATION)
				.buttons("OK", "Maybe", "Cancel").defaultButton(1);
	...
	private MessageDialogObserver openDialogAndObserve(MessageDialogBuilder MessageDialogFactory) {
		...
		display.asyncExec(() -> {
			...
			try {
				...
				if ((dialogShell.getChildren()[0]) instanceof Label) {
