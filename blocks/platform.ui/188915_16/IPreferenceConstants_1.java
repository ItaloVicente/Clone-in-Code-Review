...
public class LargeFileAssociationsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage, Listener {
	...
	public void promptForEditor() {
		...
		if (editorForSizeDialog.open() == Window.OK) {
			...
			if (!editorForSizeDialog.promptForEditor) {
				...
				editor = promptForEditorWithDialog(filteredEditors);
				if (editor == null) {
					return;
				}
			}
	...
	void editSelectedEditor() {
		...
		boolean promptForEditor = LargeFileAssociationsPreferencePage_EditorTable_DialogPrompt.equals(editorString);
	...
	private EditorDescriptor promptForEditorWithDialog(IEditorDescriptor[] filteredEditors) {
