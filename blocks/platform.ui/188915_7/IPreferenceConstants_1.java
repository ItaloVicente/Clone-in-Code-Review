...
import static org.eclipse.ui.internal.ide.IDEWorkbenchMessages.LargeFileAssociationsPreferencePage_EditorDialog_Label_DocumentSize;
...
public class LargeFileAssociationsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage, Listener {
	...
	private void setDefaultPreferenceWidgetValues() {
		String defaultPreferenceValue = LargeFileLimitsPreferenceHandler.getDefaultPreferenceValues();
	...
	protected void fillEditorTable() {
		...
		LargeFileAssociation association = getSelectedLargeDocumentAssociation();
		if (association != null) {
	...
	protected void fillResourceTypeTable() {
		...
		for (String extension : extensions) {
			String[] preferenceValues = LargeFileLimitsPreferenceHandler.getExtensionPreferenceValues(extension);
	...
	protected LargeFileAssociation getSelectedLargeDocumentAssociation() {
	...
	private void updateDefaultPreference() {
		...
		if (isDefaultPreferenceEnabled) {
			String sizeInBytes = getBytesString(defaultPreferenceFileSize, defaultPreferenceUnit);
			LargeFileLimitsPreferenceHandler.setDefaultPreferenceValue(sizeInBytes);
		} else {
			LargeFileLimitsPreferenceHandler.removeDefaultPreference();
		}
	}
	...
	private void updateExtensionPreferences() {
		...
		for (TableItem item : items) {
			...
			for (EditorForSize editorForSize : association.editors) {
				String size = getBytesString(editorForSize.size);
				String editorId = editorForSize.editor.getId();
				preferenceValues.add(size);
				preferenceValues.add(editorId);
			}
			if (association.promptSize != null) {
				String size = getBytesString(association.promptSize);
				preferenceValues.add(size);
				preferenceValues.add(LargeFileLimitsPreferenceHandler.PROMPT_EDITOR_PREFERENCE_VALUE);
			}
			if (!preferenceValues.isEmpty()) {
				...
				LargeFileLimitsPreferenceHandler.setExtensionPreferenceValues(extension,
		...
		for (String removedExtension : removedExtensions) {
			LargeFileLimitsPreferenceHandler.removeExtensionPreference(removedExtension);
		}
	}

	public void promptForEditor() {
		LargeFileAssociation association = getSelectedLargeDocumentAssociation();
		long[] configuredFileSizes = getConfiguredFileSizes(association);
	...
	private long[] getConfiguredFileSizes(LargeFileAssociation association) {
	...
	private static String getSizeText(SizeAndUnit size) {
		String sizeText = "> " + size.sizeString; //$NON-NLS-1$
		if (size.unitIndex != BYTE_UNIT_INDEX) {
			String unitText = SIZE_UNITS[size.unitIndex];
			sizeText += " " + unitText; //$NON-NLS-1$
		}
		return sizeText;
	}
	...
	private static String getBytesString(Text documentSizeText, Combo sizeUnitCombo) {
		int sizeUnitSelectionIndex = sizeUnitCombo.getSelectionIndex();
		String documentSizeString = documentSizeText.getText();
		SizeAndUnit size = new SizeAndUnit(documentSizeString, sizeUnitSelectionIndex);
		return getBytesString(size);
	}

	private static String getBytesString(SizeAndUnit size) {
		return String.valueOf(size.bytes);
	}

	private static SizeAndUnit getSizeAndUnit(Text fileSizeText, Combo sizeUnitCombo) {
		String documentSizeString = fileSizeText.getText();
		int sizeUnitSelectionIndex = sizeUnitCombo.getSelectionIndex();
		SizeAndUnit size = new SizeAndUnit(documentSizeString, sizeUnitSelectionIndex);
		return size;
	}
	...
	private static class EditorForSizeDialog extends TitleAreaDialog {
		...
		EditorForSizeDialog(Shell shell, long[] configuredFileSizes) {
			...
			size = new SizeAndUnit(DEFAULT_SIZE_STRING, MEGABYTE_UNIT_INDEX);
		...
		protected Control createDialogArea(Composite parent) {
			...
			fileSizeGroup.setText(LargeFileAssociationsPreferencePage_EditorDialog_Label_DocumentSize);
	...
	private static class SizeAndUnit {
		...
		SizeAndUnit(String sizeString, int unitIndex) {
			...
			bytes = bytesWithoutUnit * unitMultiplier;
		}
	}
}
