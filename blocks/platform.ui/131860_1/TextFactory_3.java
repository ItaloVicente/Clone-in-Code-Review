...
public class TextFactory extends ControlFactory<TextFactory, Text> {
	...
	private TextFactory(int style) {
		super(TextFactory.class, style);
	}

	public static TextFactory newText(int style, String text) {
		return new TextFactory(style).text(text);
	}
	...
	public TextFactory onWidgetSelected(Consumer<SelectionEvent> consumer) {
		this.selectionListeners.add(SelectionListener.widgetSelectedAdapter(consumer));
		return this;
	}

	public TextFactory addModifyListener(ModifyListener listener) {
		this.modifyListeners.add(listener);
		return this;
	}

	public TextFactory addVerifyListener(VerifyListener listener) {
		this.verifyListeners.add(listener);
		return this;
	}

	@Override
	protected Text createControl(Composite parent, int style) {
