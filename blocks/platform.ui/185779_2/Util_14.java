...
public class PerspectivePersisterImpl implements PerspectivePersister {
	...
	public String serializePerspectiveAndPartStates(final String perspectiveName) {
		final MWindow mWindow = Util.getCurrentMainWindow();
	...
	public void restoreWorkbenchState(final WorkbenchState workbenchState) {
		final MWindow currentWindow = Util.getCurrentMainWindow();
		...
		try {
			...
			for (final PartMemento viewSetting : workbenchState.getViewSettings()) {
				final List<MPart> elements = Util.getEModelService().findElements(currentWindow,
				...
				final Optional<IViewPart> viewPart = Util.getOptionalViewPart(part);
			...
			for (final PartMemento viewSetting : workbenchState.getViewSettings()) {
				...
				part.getPersistedState().put(Util.MEMENTO_KEY, viewSetting.getMemento());
			}
			...
			MPerspective appliedPerspective = Util.getMainPerspectiveFromWindow(currentWindow,
					workbenchState.getPerspective().getElementId());
			List<MPartStack> stacks = Util.getEModelService().findElements(appliedPerspective, null, MPartStack.class);
	...
	private void disposeCurrentParts(Collection<MPart> parts) {
		...
		parts.forEach(part -> {
			Optional<IViewPart> viewPart = Util.getOptionalViewPart(part);
			if (viewPart.isPresent()) {
				viewPartsToHide.add(viewPart.get());
				return;
			}
	...
	private void replaceSideTrimBars(MTrimmedWindow window, List<MTrimBar> trimBars) {
		for (MTrimBar trimBar : trimBars) {
			...
			MTrimBar clonedTrimBar = (MTrimBar) Util.getEModelService().cloneElement(trimBar, null);
			...
			if (existingTrimOptional.isPresent()) {
				...
				children.forEach(Util.getEModelService()::deleteModelElement);
	...
	private boolean isTrimPartOfPerspective(MTrimElement te, MTrimmedWindow window) {
		EModelService eModelService = Util.getEModelService();
	...
	private void replaceExistingPerspective(final MWindow currentWindow, final MPerspective toBeMerged,
		...
		MUIElement editorArea = Util.getEModelService().find(Util.EDITOR_AREA, currentPerspective);
		if (editorArea != null) {
	...
	private void replaceEditorAreaContents(final MWindow currentWindow, final MArea toBeMerged) {
		List<MArea> editorAreaList = Util.getEModelService().findElements(currentWindow, Util.EDITOR_AREA, MArea.class,
				null, EModelService.OUTSIDE_PERSPECTIVE | EModelService.IN_SHARED_AREA);
		final MSnippetContainer snippetContainer = Util.createSnippetContainer();
		snippetContainer.getSnippets().add(toBeMerged);
		final MArea cloneSnippet = (MArea) Util.getEModelService().cloneSnippet(snippetContainer,
	...
	private void activateEditors(MArea editorArea) {
		...
		final List<MPartStack> partStacks = Util.getEModelService().findElements(editorArea, null, MPartStack.class);
		...
		final List<MPart> editorParts = Util.getEModelService().findElements(editorArea, null, MPart.class);
	...
	private MPerspective addPerspective(final MPerspective perspective2BeMerged,
		...
		final MUIElement cloneSnippet = Util.getEModelService().cloneSnippet(snippetContainer,
