		private static Duration getFsResolution(FileStore s
			...
			for (long interval : tries) {
				...
				if (t2.compareTo(t1) > 0) {
					Duration diff = Duration.between(t1i
			...
			while (high > low) {
				...
				if (cmp > 0) {
					...
					if (diff.isZero() || diff.isNegative()) {
						LOG.warn(
								"{}: inconsistent duration from file timestamps on {}
										+ "Aborting measurement at resolution {}"
						...
						LOG.warn(
								"{}: inconsistent duration from file timestamps on {}
										+ "diff = {} > {} (last good value). Aborting measurement."
								Thread.currentThread()
						break;
