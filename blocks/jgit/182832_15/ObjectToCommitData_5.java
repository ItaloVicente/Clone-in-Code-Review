...
public class CommitGraphWriter {
	...
	private void writeHeader(CommitGraphOutputStream out
			throws IOException {
		out.writeFileHeader(getVersion()
		out.flush();
	}

	private void writeChunkLookup(CommitGraphOutputStream out
			throws IOException {
	...
	private void writeChunks(CommitGraphOutputStream out
			throws IOException {
		for (int i = 0; i < chunks.length; i++) {
