...
public class ReftableBatchRefUpdate extends BatchRefUpdate {
	...
	public void execute(RevWalk rw
		...
		try {
			if (!checkObjectExistence(rw
				return;
			}
			if (!checkNonFastForwards(rw
				return;
			}
			...
			pending.get(0).setResult(LOCK_FAILURE
			ReceiveCommand.abort(pending);
		}
	}

	private List<ReceiveCommand> getPending() {
		return ReceiveCommand.filter(getCommands()
	}
	...
	private boolean checkConflicting(List<ReceiveCommand> pending)
		...
		for (ReceiveCommand cmd : pending) {
			...
			if (refdb.isNameConflicting(name)) {
				...
				while (0 < s) {
					if (names.contains(name.substring(0
						cmd.setResult(LOCK_FAILURE);
						ok = false;
						break;
					}
	...
	private boolean checkExpected(Reftable table
			throws IOException {
		for (ReceiveCommand cmd : pending) {
			Ref ref;
	...
	private static boolean matchOld(ReceiveCommand cmd
		if (ref == null) {
			return AnyObjectId.equals(ObjectId.zeroId()
	...
	private void applyUpdates(RevWalk rw
			throws IOException {
	...
	private ReftableWriter.Stats write(OutputStream os
			long updateIndex
			throws IOException {
