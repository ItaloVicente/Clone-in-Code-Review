...
public class Applier implements InCoreSupport {
  ...
  private void apply(String path
    ...
    try (TreeWalk walk = new TreeWalk(repo)) {
      ...
      if (PatchType.GIT_BINARY.equals(fh.getPatchType())) {
        resultStreamLoader = applyBinary(repo
            fileStreamSupplier
            checkOutMetadata);
        safeWrite = true;
      } else {
        RawText raw = inCoreHandler.getRawText(walk
            convertCrLf);
        resultStreamLoader = applyText(path
        safeWrite = false;
        if (!inCore) {
          repo.getFS().setExecute(f
        }
      }
      inCoreHandler.update(resultStreamLoader
          safeWrite
  ...
  private StreamLoader applyBinary(Repository repository
      File f
  ...
  private StreamLoader applyText(String path
      checkOutMetadata)
      throws IOException
