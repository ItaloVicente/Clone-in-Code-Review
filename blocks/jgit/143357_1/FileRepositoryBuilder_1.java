...
public abstract class FSRepositoryBuilder<B extends FSRepositoryBuilder
	...
	private static File getSymRef(File workTree
		...
		File gitdirFile = fs.resolve(workTree
	...
	protected void setupGitDir() throws IOException {
		if (getGitDir() == null && getWorkTree() != null) {
			File dotGit = new File(getWorkTree()
			if (!dotGit.isFile())
				setGitDir(dotGit);
			else
				setGitDir(getSymRef(getWorkTree()
		}
	}
