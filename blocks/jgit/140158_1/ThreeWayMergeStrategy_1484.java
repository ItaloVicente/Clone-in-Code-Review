...
public class StrategySimpleTwoWayInCore extends ThreeWayMergeStrategy {
	...
	private static class InCoreMerger extends ThreeWayMerger {
		...
		InCoreMerger(Repository local) {
			super(local);
			tw = new NameConflictTreeWalk(local
			cache = DirCache.newInCore();
		}

		InCoreMerger(ObjectInserter inserter) {
			super(inserter);
			tw = new NameConflictTreeWalk(null
			cache = DirCache.newInCore();
		}
		...
		protected boolean mergeImpl() throws IOException {
			...
			while (tw.next()) {
				...
				if (modeB == modeO && tw.idEqual(T_BASE
					add(T_THEIRS
				else if (modeB == modeT && tw.idEqual(T_BASE
					add(T_OURS
				else {
					if (nonTree(modeB)) {
						add(T_BASE
