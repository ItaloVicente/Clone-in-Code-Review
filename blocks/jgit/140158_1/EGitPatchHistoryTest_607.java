...
public class CGitVsJGitRandomIgnorePatternTest {
	...
	public static class CgitFatalException extends Exception {
		...
		public CgitFatalException(int cgitExitCode
				String cgitStdError) {
	...
	public static class CGitIgnoreRule {
		...
		public CGitIgnoreRule(File gitDir
			this.gitDir = gitDir;
			this.pattern = pattern;
			Files.write(FileUtils.toPath(new File(gitDir
					(pattern + "\n").getBytes(UTF_8)
		...
		public boolean isMatch(String path)
			...
			if (cgitExitCode == 128) {
				throw new CgitFatalException(cgitExitCode
						cgitStdError);
			}
	...
	public void testRandomPatterns() throws Exception {
		...
		for (int i = 0; i < NB_PATTERN; i++) {
			...
			CGitIgnoreRule cgitIgnoreRule = new CGitIgnoreRule(gitDir

			assertCgitAndJgitMatch(pattern
					pattern);
	...
	private void assertCgitAndJgitMatch(String pattern
			FastIgnoreRule jgitIgnoreRule
			String pathToTest) throws IOException
	...
	private String buildAssertionToAdd(String pattern
			boolean cgitMatch) {
		return "assertMatch(" + toJavaString(pattern) + "
				+ toJavaString(pathToTest) + "
	}

	private String toJavaString(String pattern2) {
		return "\"" + pattern2.replace("\\"
				+ "\"";
	}
}
