...
public class BitmapIndexImpl implements BitmapIndex {
	...
	int findOrInsert(AnyObjectId objectId
		int position = findPosition(objectId);
		if (position < 0) {
			position = mutableIndex.findOrInsert(objectId
			position += indexObjectCount;
		}
		return position;
	}
	...
	private static final class CompressedBitmapBuilder implements BitmapBuilder {
		...
		public BitmapBuilder addObject(AnyObjectId objectId
			bitset.set(bitmapIndex.findOrInsert(objectId
			return this;
		}
	...
	public static final class CompressedBitmap implements Bitmap {
		...
		public CompressedBitmap or(Bitmap other) {
			return new CompressedBitmap(bitmap.or(ewahBitmap(other))
		}

		@Override
		public CompressedBitmap andNot(Bitmap other) {
			return new CompressedBitmap(bitmap.andNot(ewahBitmap(other))
		}

		@Override
		public CompressedBitmap xor(Bitmap other) {
			return new CompressedBitmap(bitmap.xor(ewahBitmap(other))
		}

		private final IntIterator ofObjectType(int type) {
			return bitmapIndex.packIndex.ofObjectType(bitmap
		}
