...
class PackBitmapIndexV1 extends BasePackBitmapIndex {
	...
	PackBitmapIndexV1(final InputStream fd
		...
		final int version = NB.decodeUInt16(scratch
		if (version != 1)
			throw new IOException(MessageFormat.format(
					JGitText.get().unsupportedPackIndexVersion
					Integer.valueOf(version)));

		final int opts = NB.decodeUInt16(scratch
		if ((opts & OPT_FULL) == 0)
			throw new IOException(MessageFormat.format(
					JGitText.get().expectedGot
					Integer.valueOf(opts)));

		long numEntries = NB.decodeUInt32(scratch
		...
		for (int i = 0; i < (int) numEntries; i++) {
			IO.readFully(fd
			int nthObjectId = NB.decodeInt32(scratch
