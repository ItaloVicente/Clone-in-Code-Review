...
public abstract class BasePackPushConnection extends BasePackConnection implements
	...
	private String enableCapabilities(final ProgressMonitor monitor
		...
		if (atomic)
			capableAtomic = wantCapability(line
		capableReport = wantCapability(line
		capableDeleteRefs = wantCapability(line
		capableOfsDelta = wantCapability(line

		if (pushOptions != null) {
			capablePushOptions = wantCapability(line
		}
	...
	private void writePack(final Map<String
	...
	private void readStatusReport(Map<String
			throws IOException {
		final String unpackLine = readStringLongTimeout();
			throw new PackProtocolException(uri
					.format(JGitText.get().unexpectedReportLine
			throw new TooLargePackException(uri
			throw new TooLargeObjectInPackException(uri
			throw new TransportException(uri
					JGitText.get().errorOccurredDuringUnpackingOnTheRemoteEnd
		}
		...
		for (RemoteRefUpdate rru : refUpdates.values()) {
			if (rru.getStatus() == Status.AWAITING_REPORT)
				throw new PackProtocolException(MessageFormat.format(
						JGitText.get().expectedReportForRefNotReceived 
		}
	}
	...
	private String readStringLongTimeout() throws IOException {
		...
		final int sendTime = (int) Math.min(packTransferTime
		try {
			int timeout = 10 * Math.max(sendTime
