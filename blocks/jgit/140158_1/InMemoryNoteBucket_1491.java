...
class FanoutBucket extends InMemoryNoteBucket {
	...
	InMemoryNoteBucket set(AnyObjectId noteOn
		...
		if (b == null) {
			...
			table[cell] = n.set(noteOn
			cnt++;
			return this;

		} else {
			NoteBucket n = b.set(noteOn
			if (n == null) {
				table[cell] = null;
				cnt--;
				...
				return contractIfTooSmall(noteOn
	...
	private class LazyNoteBucket extends NoteBucket {
		...
		@Override
		int estimateSize(AnyObjectId objId
			return load(objId
		}
		...
		private InMemoryNoteBucket load(AnyObjectId prefix
				throws IOException {
			AbbreviatedObjectId p = prefix.abbreviate(prefixLen + 2);
			InMemoryNoteBucket self = NoteParser.parse(p
