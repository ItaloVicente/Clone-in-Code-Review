...
final class HistogramDiffIndex<S extends Sequence> {
		...
		SCAN: for (int ptr = region.endA - 1; region.beginA <= ptr; ptr--) {
			...
			for (int rIdx = table[tIdx]; rIdx != 0;) {
				...
				if (cmp.equals(a
					...
					if (MAX_CNT < newCnt)
						newCnt = MAX_CNT;
					recs[rIdx] = recCreate(recNext(rec)
			...
			if (rIdx == recs.length) {
				int sz = Math.min(recs.length << 1
				long[] n = new long[sz];
				System.arraycopy(recs
				recs = n;
			}

			recs[rIdx] = recCreate(table[tIdx]
			recIdx[ptr - ptrShift] = rIdx;
			table[tIdx] = rIdx;
		}
		return true;
	...
	private int tryLongestCommonSequence(int bPtr) {
		int bNext = bPtr + 1;
		int rIdx = table[hash(b
		for (long rec; rIdx != 0; rIdx = recNext(rec)) {
			rec = recs[rIdx];

			if (recCnt(rec) > cnt) {
				if (!hasCommon)
					hasCommon = cmp.equals(a
				continue;
			}
	...
	private int hash(HashedSequence<S> s
		return (cmp.hash(s
	}
