...
public class ObjectChecker {
	...
	public ObjectChecker setAllowLeadingZeroFileMode(boolean allow) {
		return setIgnore(ZERO_PADDED_FILEMODE
	}
	...
	public void check(@Nullable AnyObjectId id
			throws CorruptObjectException {
		switch (objType) {
		case OBJ_COMMIT:
			checkCommit(id
	...
	public void checkCommit(@Nullable AnyObjectId id
			throws CorruptObjectException {
		bufPtr.value = 0;
		...
		if (match(raw
			checkPersonIdent(raw
		} else {
			report(MISSING_AUTHOR
		}

		if (match(raw
			checkPersonIdent(raw
		} else {
			report(MISSING_COMMITTER
					JGitText.get().corruptObjectNoCommitter);
		}
	}

	public void checkTag(byte[] raw) throws CorruptObjectException {
		checkTag(idFor(OBJ_TAG
	}

	public void checkTag(@Nullable AnyObjectId id
			throws CorruptObjectException {
		bufPtr.value = 0;
		if (!match(raw
	...
	public void checkTree(byte[] raw) throws CorruptObjectException {
		checkTree(idFor(OBJ_TREE
	}

	public void checkTree(@Nullable AnyObjectId id
			throws CorruptObjectException {
		final int sz = raw.length;
		int ptr = 0;
		int lastNameB = 0
		...
		while (ptr < sz) {
			...
			if (ObjectId.zeroId().compareTo(raw
				report(NULL_SHA1
			}

			if (id != null && isGitmodules(raw
				ObjectId blob = ObjectId.fromRaw(raw
				gitsubmodules.add(new GitmoduleEntry(id
			}
		}
	}
	...
	private void checkPathSegment2(byte[] raw
		...
		if (raw[ptr] == '.') {
			...
			case 4:
				...
				break;
			default:
				if (end - ptr > 4 && isNormalizedGit(raw
					report(HAS_DOTGIT
							JGitText.get().corruptObjectInvalidName
							RawParseUtils.decode(raw
				}
			}
		} else if (isGitTilde1(raw
			report(HAS_DOTGIT
					JGitText.get().corruptObjectInvalidName
					RawParseUtils.decode(raw
		}
		if (macosx && isMacHFSGit(raw
			report(HAS_DOTGIT
	...
	private boolean isMacHFSPath(byte[] raw
			@Nullable AnyObjectId id) throws CorruptObjectException {
		boolean ignorable = false;
		int g = 0;
		while (ptr < end) {
			switch (raw[ptr]) {
				if (!checkTruncatedIgnorableUTF8(raw
					return false;
				}
				switch (raw[ptr + 1]) {
				case (byte) 0x80:
	...
	private boolean checkTruncatedIgnorableUTF8(byte[] raw
			@Nullable AnyObjectId id) throws CorruptObjectException {
		if ((ptr + 2) >= end) {
			report(BAD_UTF8
	...
	private void checkNotWindowsDevice(byte[] raw
			@Nullable AnyObjectId id) throws CorruptObjectException {
	...
	private String normalize(byte[] raw
		...
		return macosx ? Normalizer.normalize(n
	}
