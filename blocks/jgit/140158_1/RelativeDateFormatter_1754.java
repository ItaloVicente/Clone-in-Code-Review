...
public class RefMap extends AbstractMap<String
	final String prefix;
	...
	public Set<Entry<String
		if (entrySet == null) {
			entrySet = new AbstractSet<Entry<String
				@Override
	...
	private class SetIterator implements Iterator<Entry<String
		...
		public Entry<String
			if (packedIdx < packed.size() && looseIdx < loose.size()) {
				...
				int cmp = RefComparator.compareTo(p
				if (cmp < 0) {
					packedIdx++;
					return toEntry(p);
				}
		...
		private Ref resolveLoose(Ref l) {
			if (resolvedIdx < resolved.size()) {
				...
				int cmp = RefComparator.compareTo(l
				if (cmp == 0) {
					resolvedIdx++;
					return r;
				} else if (cmp > 0) {
	...
	private class Ent implements Entry<String
		private Ref ref;
			...
			if (obj instanceof Map.Entry) {
				...
				if (key instanceof String && val instanceof Ref) {
					...
					if (r.getName().equals(ref.getName())) {
						...
						if (a != null && b != null && AnyObjectId.equals(a
							return true;
					}
				}
			}
