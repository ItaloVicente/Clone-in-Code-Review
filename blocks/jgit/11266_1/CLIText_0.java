...
public class PseudorandomTest extends CLIRepositoryTestCase {
	...
	private void initRepository() throws IOException
		FileRepository[] dbs = new FileRepository[] { db
		for (int i = 0; i < dbs.length; i++) {
			JGitTestUtil.writeTrashFile(dbs[i]
	...
	@Theory
	public void jgitProducesSameOutputAsNativeGit(String cmd1
			throws Exception {
		Assume.assumeTrue(isNativeGitAvailable());

		String[] cmds = new String[] { cmd1
		for (String cmd : cmds) {
	...
	private List<String> executeCGit(final String cmd) throws IOException {
		...
		Process p = Runtime.getRuntime().exec(GIT_BIN_PATH + cmd
