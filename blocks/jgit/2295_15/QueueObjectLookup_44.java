...
class Prefetcher implements StreamingCallback<Collection<PackChunk.Members>> {
	...
	void push(DhtReader ctx
		...
		toFind.add(ObjectIndexKey.create(ctx.getRepositoryKey()
		toFind.add(ObjectIndexKey.create(ctx.getRepositoryKey()
		db.objectIndex().get(Context.READ_REPAIR
		...
		if (startKey == null)
			throw DhtReader.missing(start

		ChunkKey endKey = chunk(trees.get(end));
		if (endKey == null)
			throw DhtReader.missing(end
	...
	synchronized ChunkAndOffset find(
			@SuppressWarnings("hiding") RepositoryKey repo
