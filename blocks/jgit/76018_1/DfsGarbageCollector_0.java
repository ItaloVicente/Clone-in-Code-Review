...
public class DfsGarbageCollectorTest {
	...
	public void testCollectionWithGarbageAndGarbagePacksPurged()
		...
		for (DfsPackFile pack : odb.getPacks()) {
			...
			if (d.getPackSource() == GC) {
				...
				commit1Found |= isObjectInPack(commit1
			} else {
				fail("unexpected " + d.getPackSource());
			}
		}
		assertTrue("garbage commit1 still readable"

		DfsPackDescription oldestGarbagePack = null;
		for (DfsPackFile pack : odb.getPacks()) {
			DfsPackDescription d = pack.getPackDescription();
			if (d.getPackSource() == UNREACHABLE_GARBAGE) {
				oldestGarbagePack = oldestPack(oldestGarbagePack
			}
		}
		assertNotNull("has UNREACHABLE_GARBAGE"

		gcWithTtl();
		assertTrue("has packs"
		for (DfsPackFile pack : odb.getPacks()) {
			assertNotEquals(oldestGarbagePack
		}
	}
	...
	private void gcWithTtl() throws InterruptedException
