	...
	public void load(ConfigProperties systemConfig) {
		...
		final ConfigProperties.ConfigProperty jgitFileSystemsCacheOverflowSizePropCacheProp = systemConfig
				.get(JGIT_CACHE_OVERFLOW_CLEANUP_SIZE

		final ConfigProperties.ConfigProperty jgitRemoveEldestEntryIterationsProp = systemConfig
				.get(JGIT_REMOVE_ELDEST_ENTRY_ITERATIONS

		final ConfigProperties.ConfigProperty jgitCacheEvictThresoldDurationProp = systemConfig
				.get(JGIT_CACHE_EVICT_THRESHOLD_DURATION

		final ConfigProperties.ConfigProperty jgitCacheEvictThresoldTimeUnitProp = systemConfig
				.get(JGIT_CACHE_EVICT_THRESHOLD_TIME_UNIT

		final ConfigProperties.ConfigProperty jgitSshCiphers = systemConfig.get(GIT_SSH_CIPHERS
		final ConfigProperties.ConfigProperty jgitSshMacs = systemConfig.get(GIT_SSH_MACS
		...
		if (sshEnabled) {
			...
			try {
				...
				LOG.error(
						"SSH Idle Timeout value is not a valid integer - Parameter is ignored
