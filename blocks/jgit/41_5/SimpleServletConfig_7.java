...
class ServletDefinition {
	...
	ServletDefinition(final String pattern
	...
	void init(final ServletContext context) throws ServletException {
		...
		servlet.init(new SimpleServletConfig(name
	}
	...
	void service(final HttpServletRequest req
			throws IOException
		...
		try {
			if (servlet instanceof RepositoryServlet) {
				...
				RepositoryServlet.setRepository(req
			}

			servlet.service(new WrappedRequest(req
		} finally {
			if (db != null)
				db.close();
		}
