...
public class PatchApplier implements InCoreSupport {
  ...
  public PatchApplier(Repository remote
    repo = remote;
    inCore = true;
    this.tip = tip;
    inserter = oi;
  ...
  public ApplyPatchResult applyPatch(InputStream patchInput)
    ...
    try {
      ...
      ioHandler = InCoreSupport.initRepoIOHandler(repo
      ...
      for (org.eclipse.jgit.patch.FileHeader fh : p.getFiles()) {
        ...
        switch (type) {
          ...
          case DELETE:
            f = inCore ? null : getFile(fh.getOldPath()
            ioHandler.deleteFile(fh.getOldPath()
            break;
          case RENAME:
            f = inCore ? null : getFile(fh.getOldPath()
            File dest = getFile(fh.getNewPath()
            ioHandler.renameFile(f
            apply(fh.getOldPath()
            break;
          case COPY:
            f = inCore ? null : getFile(fh.getOldPath()
            File target = getFile(fh.getNewPath()
            ioHandler.copyFile(f
            apply(fh.getOldPath()
        }
        if (f != null) {
  ...
  private File getFile(String path
    File f = new File(ioHandler.nonNullRepo().getWorkTree()
    if (create) {
      try {
  ...
  private void apply(String path
    ...
    try {
      ...
      if (PatchType.GIT_BINARY.equals(fh.getPatchType())) {
        ...
        RawText raw = ioHandler.getRawText(f
            filterCommand
        resultStreamLoader = applyText(raw
        safeWrite = false;
        if (!inCore) {
          ioHandler.nonNullRepo().getFS().setExecute(f
        }
      }

      CheckoutMetadata checkOutMetadata = new CheckoutMetadata(streamType
      if (resultStreamLoader != null) {
        ioHandler.updateFileWithContent(resultStreamLoader
            safeWrite
      }
      if (hash != null) {
  ...
  private StreamLoader applyBinary(String path
      RepoIOHandler.StreamSupplier inputSupplier
      throws PatchApplyException
    if (inCore) {
    ...
    switch (hunk.getType()) {
      case LITERAL_DEFLATED:
        ...
        return ioHandler.createStreamLoader(() -> inflated
      case DELTA_DEFLATED:
        byte[] base;
        ...
        return ioHandler.createStreamLoader(() -> hashed
      default:
        throw new UnsupportedOperationException(
  ...
  private StreamLoader applyText(RawText rt
    ...
    try (OutputStream out = buffer) {
      ...
      return ioHandler.createStreamLoader(buffer::openInputStream
    }
  }
