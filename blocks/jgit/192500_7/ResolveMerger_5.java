	...
	private TreeWalk getRenameWalk(RevTree[] renameTrees
	...
	private TreeWalk getRenameWalk(RevTree renameTree
		return  getRenameWalk(new RevTree[]{renameTree}
	}
	...
	private void setUpRenameWalk(RenameProcessingTreeWalk walk
		...
		TreeWalk swapWalk = getRenameWalk(renameTrees
	...
	private boolean processRenames(RevTree baseTree
		...
		for (Entry<String
			...
			if (renameType.equals(RENAME_BOTH_SIDES_CONFLICT) || this.renameResolver.conflictingRenamePath.containsKey(baseRename.getKey())) {
				reportRenameConflict(baseRename.getKey()
				continue;
			}
	...
	private void reportConflict(String path
		...
		TreeWalk treeWalk = getRenameWalk(new RevTree[]{base
		if (ignoreConflicts) {
		...
		public AbstractTreeIterator[] swapRenames(AbstractTreeIterator base
			...
			if (isBaseRename(base)) {
				...
				if (paths.containsKey(T_OURS) && !paths.containsKey(T_THEIRS)) {
					canonicalTreeParsers = new AbstractTreeIterator[]{null
					...
					conflictingRenamePath.put(base.getEntryPathString()
					return new AbstractTreeIterator[]{null
				}
			}
			else if (isRenameFromBase(T_OURS

				conflictingRenamePath.put(getOriginalByRename(T_OURS
				return new AbstractTreeIterator[]{null
			}
			else if (isRenameFromBase(T_THEIRS
				conflictingRenamePath.put(getOriginalByRename(T_THEIRS
				return new AbstractTreeIterator[]{null
			}
