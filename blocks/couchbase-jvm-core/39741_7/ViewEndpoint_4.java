...
public class QueryHandler extends AbstractGenericHandler<HttpObject, HttpRequest, QueryRequest> {
        ...
        while(true) {
            ...
            for (int i = responseContent.readerIndex(); i <= responseContent.writerIndex(); i++) {
                ...
                if(current == '"') {
                    if (!inString) {
                        inString = true;
                    } else if(responseContent.getByte(i-1) != '\\') {
                        inString = false;
                    }
                } else if (current == '{' && !inString) {
                    openBraces++;
                } else if (current == '}' && openBraces > 0 && !inString) {
                    openBraces--;
                    if (openBraces == 0) {
                        closeBracePos = i;
                        break;
            ...
            int to = closeBracePos - openBracketPos - responseContent.readerIndex() + 1;
            queryRowObservable.onNext(responseContent.slice(from, to).copy());
            responseContent.readerIndex(closeBracePos);
        }
    ...
    private void parseQueryInfo(boolean last) {
        ...
        while(true) {
            final int openBracketPos = responseContent.bytesBefore((byte) '{');
