        updateConnectionStats(ctx, msg);
    }

    private void updateConnectionStats(ChannelHandlerContext ctx, FullBinaryMemcacheResponse msg) {
        final DCPStream stream = streams.get(msg.getOpaque());
        final DCPConnection connection = connections.get(stream.connectionName());
