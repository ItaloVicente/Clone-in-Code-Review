...
import com.couchbase.client.core.message.kv.subdoc.SubDeleteRequest;
...
import com.couchbase.client.core.message.kv.subdoc.SubReplaceRequest;
...
public class SubdocumentMessageTest extends ClusterDependentTest {
    ...
    public void shouldDeleteExistingArrayIndexPath() {
        ...
        String expected = "{\"value\":\"stringValue\", \"sub\": {\"value\": \"subStringValue\",\"array\": [\"array1\", true]}}";

        GetResponse finalState = cluster().<GetResponse>send(new GetRequest(testSubKey, bucket())).toBlocking().single();
        ReferenceCountUtil.releaseLater(finalState.content());
        String actual = finalState.content().toString(CharsetUtil.UTF_8).replaceAll(" +", " ");
    ...
    public void shouldNotAllowEmptyPathInDelete() {
        String path = "";
        SubDeleteRequest request = new SubDeleteRequest(testSubKey, path, bucket());
        SimpleSubdocResponse response = cluster().<SimpleSubdocResponse>send(request).toBlocking().single();
        ReferenceCountUtil.releaseLater(response.content());
    ...
    public void shouldReplaceEntryInArray() {
        ...
        String expected = "{\"value\":\"stringValue\", \"sub\": {\"value\": \"subStringValue\",\"array\": [\"mutated\", 2, true]}}";
