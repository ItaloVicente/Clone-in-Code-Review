...
public class SlowOperationSpanBuilder implements Tracer.SpanBuilder {
    ...
    public Tracer.SpanBuilder addReference(final String referenceType, final SpanContext referencedContext) {
        ...
        if (references.isEmpty()) {
            references = Collections.singletonList(new SlowOperationReference(referencedContext, referenceType));
        } else {
            if (references.size() == 1) {
                references = new ArrayList<SlowOperationReference>(references);
            }
            references.add(new SlowOperationReference(referencedContext, referenceType));
        }
    ...
    @Override
    public Span start() {
        SlowOperationSpanContext context = new SlowOperationSpanContext();
