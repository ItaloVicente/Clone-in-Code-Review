...
public class SlowOperationReporter {
    ...
    private static final AtomicInteger iId = new AtomicInteger();
    ...
    private final Queue<CouchbaseResponse> zombieQueue;
    ...
    SlowOperationReporter(final Builder builder) {
        ...
        zombieQueue = new MpscUnboundedArrayQueue<CouchbaseResponse>(1024);
    ...
    public void report(final SlowOperationSpan span) {
        if (span.getTag("couchbase.service") != null && spanOverThreshold(span)) {
            if (!spansToTrack.offer(span)) {
                LOGGER.debug("Could not enqueue span {} for slowlog reporting, discarding.", span);
            }
        }
    }

    public void reportZombie(final CouchbaseResponse response) {
        if (!zombieQueue.offer(response)) {
            LOGGER.debug("Could not enqueue zombie response {} for reporting, discarding.", response);
        }
    }
    ...
    class Worker implements Runnable {
        ...
        @Override
        public void run() {
            Thread.currentThread().setName("cb-tracing-" + iId.incrementAndGet());
            while(true) {
                try {
        ...
        private void logSlowOps() throws Exception {
            ...
            buildSpanMap(kvSet, top);
            buildSpanMap(viewSet, top);
            buildSpanMap(n1qlSet, top);
            buildSpanMap(analyticsSet, top);
            buildSpanMap(ftsSet, top);

            result.put("count", total);
            result.put("top", top);

            LOGGER.warn("Slow operations in interval {}", JACKSON.writeValueAsString(result));
        }

        private void buildSpanMap(SortedSet<SlowOperationSpan> spanSet, final Map<String, List<String>> input) {
            Iterator<SlowOperationSpan> spanIter = spanSet.iterator();
            while(spanIter.hasNext()) {
                SlowOperationSpan span = spanIter.next();
                String remote = span.context().remoteSocket() == null
                    ? "<unknown>" : span.context().remoteSocket();
