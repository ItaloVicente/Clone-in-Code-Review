                ...
                return Observable.from(multiLookupResponse.responses())
                        .map(new Func1<com.couchbase.client.core.message.kv.subdoc.multi.MultiResult<Lookup>, MultiResult<Lookup>>() {
                            ...
                            public MultiResult<Lookup> call(com.couchbase.client.core.message.kv.subdoc.multi.MultiResult<Lookup> lookupResult) {
                                ...
                                try {
                                    if (isExist) {
                                        return MultiResult.createResult(path, lookupResult.operation(), lookupResult.status(), success);
                                    } else if (success) {
                                        try {
                                            Object content = subdocumentTranscoder.decode(lookupResult.value(), Object.class);
                                            return MultiResult.createResult(path, Lookup.GET, lookupResult.status(), content);
                                        } catch (TranscodingException e) {
                                            LOGGER.error("Couldn't decode multi-lookup " + lookupResult.operation() + " for " + docId + "/" + path, e);
                                            return MultiResult.createFatal(path, lookupResult.operation(), e);
                                        }
                                    } else {
                                        return MultiResult.createResult(path, Lookup.GET, lookupResult.status(), null);
                                    }
                                } finally {
    ...
    private <T> Observable<DocumentFragment<Lookup>> getIn(final String id, final String path, final Class<T> fragmentType) {
        return Observable.defer(new Func0<Observable<SimpleSubdocResponse>>() {
            ...
            public Boolean call(SimpleSubdocResponse response) {
                if (response.status().isSuccess() || response.status() == ResponseStatus.SUBDOC_PATH_NOT_FOUND) {
                    return true;
                }
            ...
            public DocumentFragment<Lookup> call(SimpleSubdocResponse response) {
                try {
                    ...
                    if (response.status().isSuccess()) {
                        content = subdocumentTranscoder.decodeWithMessage(response.content(), fragmentType,
