...
public class ReactiveCollectionImpl extends ReactiveCollection {
    ...
    @SuppressWarnings({"unchecked"})
    @Override //TODO allow expiry
    public Observable<JsonLongDocument> counter(String id, long delta, long initial, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
			return Mutate.counter(this.collectionId, id, delta, 0, 0, env, core, bucketName, timeout, timeunit, null);
        } else {
            final Span parent = startTracing("counter_with_durability");
            return addParentSpan(parent, options, timeout, timeunit, Mutate.counter(this.collectionId, id, delta, 0, 0, env, core, bucketName, timeout, timeunit, parent));
        }
    }
