...
public class JsonCryptoTranscoder extends AbstractTranscoder<JsonDocument, JsonObject> {
    ...
    private void addEncryption(JsonObject content) throws Exception {
        if (content.encryptionPathInfo().size() == 0) {
            return;
        }

        for (Map.Entry<String, String> encryptionInfo:content.encryptionPathInfo().entrySet()) {
            String[] pathSplit = encryptionInfo.getKey().split("/");

            JsonObject parent = content;
            String[] keyTypeInfo = pathSplit[pathSplit.length-1].split(":");
            String lastPointer = keyTypeInfo[0];
            String type = keyTypeInfo[1];
            ...
            EncryptionCrytoProvider provider = this.encryptionConfig.getCryptoProvider(encryptionInfo.getValue());
            if (provider == null) {
                throw new Exception("Encryption provider does not exist in the configuration");
            }

            encrypted.put("kid", provider.getKeyName());
            encrypted.put("alg", encryptionInfo.getValue());
            encrypted.put("payload", provider.encrypt(value));
            encrypted.put("type", type);
