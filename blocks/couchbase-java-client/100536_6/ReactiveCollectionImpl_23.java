...
public class ReactiveCollectionImpl extends ReactiveCollection {
    ...
    @Override
    public ReactiveCollection durabilityOptions(DurabilityOptions options) {
        this.durabilityOptions = options;
        return this;
    }

    @Override
    public ReactiveCollection kvTimeout(long timeout, TimeUnit timeunit) {
    ...
    @Override
    public ReactiveCollection expiry(int expiry) {
        this.expiry = expiry;
        return this;
    }
    ...
    public <D extends Document<?>> Observable<D> getAndTouch(String id, int expiry, Class<D> target, long timeout, TimeUnit timeunit) {
        if (this.expiry != 0 && expiry == 0) expiry = this.expiry;
    ...
    public Observable<Boolean> touch(String id, int expiry, long timeout, TimeUnit timeunit) {
        if (this.expiry != 0 && expiry == 0) expiry = this.expiry;
        return Mutate.touch(this.collectionId, id, expiry, env, core, bucketName,  timeout, timeunit);
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> insert(final D document, final DurabilityOptions options, final long timeout, final TimeUnit timeunit) {
        if (this.expiry != 0 && document.expiry() == 0) document.expiry(this.expiry);
        ...
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
            ...
            return addParentSpan(parent, options, timeout, timeunit, Mutate.insert(this.collectionId, document, env, transcoder, core, bucketName, timeout, timeunit, parent));
        }
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> upsert(final D document, final DurabilityOptions options, final long timeout, final TimeUnit timeunit) {
        if (this.expiry != 0 && document.expiry() == 0) document.expiry(this.expiry);
        final Transcoder<Document<Object>, Object> transcoder = (Transcoder<Document<Object>, Object>) transcoders.get(document.getClass());
        ...
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
            ...
            return addParentSpan(parent, options, timeout, timeunit, Mutate.upsert(this.collectionId, document, env, transcoder, core, bucketName, timeout, timeunit, parent));
        }
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> append(D document, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        if (this.expiry != 0 && document.expiry() == 0) document.expiry(this.expiry);
        final Transcoder<Document<Object>, Object> transcoder = (Transcoder<Document<Object>, Object>) transcoders.get(document.getClass());
        ...
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
            ...
            return addParentSpan(parent, options, timeout, timeunit, Mutate.append(this.collectionId, document, env, transcoder, core, bucketName, timeout, timeunit, parent));
        }
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> prepend(D document, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        if (this.expiry != 0 && document.expiry() == 0) document.expiry(this.expiry);
        final Transcoder<Document<Object>, Object> transcoder = (Transcoder<Document<Object>, Object>) transcoders.get(document.getClass());
        ...
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
            ...
            return addParentSpan(parent, options, timeout, timeunit, Mutate.prepend(this.collectionId, document, env, transcoder, core, bucketName, timeout, timeunit, parent));
        }
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> replace(D document, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        if (this.expiry != 0 && document.expiry() == 0) document.expiry(this.expiry);
        final Transcoder<Document<Object>, Object> transcoder = (Transcoder<Document<Object>, Object>) transcoders.get(document.getClass());
        ...
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
            ...
            return addParentSpan(parent, options, timeout, timeunit, Mutate.replace(this.collectionId, document, env, transcoder, core, bucketName, timeout, timeunit, parent));
        }
    }

    @SuppressWarnings({"unchecked"})
    @Override
    public Observable<JsonLongDocument> counter(String id, long delta, long initial, int expiry, long cas, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        if (this.expiry != 0 && expiry == 0) expiry = this.expiry;
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
			return Mutate.counter(this.collectionId, id, delta, initial, expiry, env, core, bucketName, timeout, timeunit, null);
        } else {
            final Span parent = startTracing("counter_with_durability");
            return addParentSpan(parent, options, timeout, timeunit, Mutate.counter(this.collectionId, id, delta, initial, expiry, env, core, bucketName, timeout, timeunit, parent));
        }
    }
    ...
    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> remove(String id, Class<D> target, DurabilityOptions options, long timeout, TimeUnit timeunit) {
    ...
    @SuppressWarnings({"unchecked"})
    @Override
    public <D extends Document<?>> Observable<D> remove(D document, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        final Transcoder<Document<Object>, Object> transcoder = (Transcoder<Document<Object>, Object>) transcoders.get(document.getClass());
        ...
        if (options == null || (options.persistTo() == PersistTo.NONE && options.replicateTo() == ReplicateTo.NONE)) {
            ...
            return addParentSpan(parent, options, timeout, timeunit, Mutate.remove(this.collectionId, document, env, transcoder, core, bucketName, timeout, timeunit, parent));
        }
    }
    ...
    private <D extends Document<?>> Observable<D> addParentSpan(final Span parent, final DurabilityOptions options, final long timeout, final TimeUnit timeunit, final Observable<D> obs) {
