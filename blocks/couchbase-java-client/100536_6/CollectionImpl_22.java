...
public class CollectionImpl extends Collection {
    ...
    protected CollectionImpl(final String name, final long cid, final Scope scope, final Bucket bucket) {
        ...
        this.reactiveCollection.kvTimeout(this.timeout, this.timeoutUnit);
        this.reactiveCollection.durabilityOptions(this.durabilityOptions);
    }

    @Override
    public Collection durabilityOptions(DurabilityOptions options) {
        this.durabilityOptions = options;
        this.reactiveCollection().durabilityOptions(options);
        return this;
    }

    @Override
    public Collection kvTimeout(long timeout, TimeUnit timeunit) {
        this.timeout = timeout;
        this.timeoutUnit = timeunit;
        this.reactiveCollection().kvTimeout(timeout, timeoutUnit);
        return this;
    }

    @Override
    public Collection expiry(int expiry) {
        this.expiry = expiry;
        this.reactiveCollection().expiry(expiry);
        return this;
    }
    ...
    @Override
    public JsonLongDocument counter(String id, long delta, long initial, int expiry, long cas, DurabilityOptions options, long timeout, TimeUnit timeunit) {
        return Blocking.blockForSingle(this.reactiveCollection.counter(id, delta, initial, expiry, cas, options, timeout, timeunit), timeout, timeunit);
    }
